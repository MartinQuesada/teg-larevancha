...
TEG_STATUS ai_fichas_calc_puntaje_conquer( int country )
{
	...
	if( pc != pm ) {
		...
		if( country_esbloqueado( &g_countries[country], WHOAMI() ) )
		{
			...
			// fprintf(stderr,_("Pais: %s, Puntaje: %d\n"), g_countries[country].name, ai_puntaje[country] );
		}
		
		// Verifico si está en mi objetivo
		//PCOUNTRY pP = &g_countries[country];
		if( country_inmission( &g_countries[country], WHOAMI() ) == TRUE ){
			ai_puntaje[country] += 10;
			//fprintf(stderr, _("País: %s, Jugador: %i. Puntaje +10.\n"),g_countries[country].name,WHOAMI() );
		}
	}
#endif
	return TEG_STATUS_SUCCESS;
...
TEG_STATUS ai_fichas_calc_puntaje_defense( int country )
{
	...
	for(i=0;i< COUNTRIES_CANT;i++) {
		...
		/* if (g_countries[i].numjug == WHOAMI() )
			fprintf(stderr, _("Evaluando defensivamente: %s . Puntaje: %i .\n"),g_countries[i].name, suma );*/
	}
...
TEG_STATUS ai_fichas_calc_puntaje( int p )
{
	...
	for(i=0;i< COUNTRIES_CANT;i++)
		if (g_countries[i].numjug == WHOAMI() )
			// fprintf(stderr, _("País: %s, Puntaje: %i .\n"),g_countries[i].name, ai_puntaje[i] );
