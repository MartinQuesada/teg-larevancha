...
BOOLEAN country_inmission( PCOUNTRY pP, int numjug )
{
	...
	//{
		fprintf(stderr, _("Pais(Jug): %s(%i), Col-Enemigo: %i, Obj-dest: %i, Obj-cont: %i\n"), pP->name,pP->numjug,pJ_enemigo->color,g_missions[g_game.secret_mission].jugadores[pJ_enemigo->color], g_missions[g_game.secret_mission].continentes[pP->continente]);
		//fprintf(stderr, _("Numjug: %i , Pais: %s, Obj: %i\nObj-name: %s"), numjug, pP->name,g_game.secret_mission, g_missions[g_game.secret_mission].name);

                // Misión común o conquistar el mundo
                if (g_game.secret_mission == MISSION_COMMON || g_game.secret_mission == MISSION_CONQWORLD)
                        return TRUE;

                // Objetivo de cant. de países
                if (g_missions[g_game.secret_mission].tot_countries != 0)
                        return TRUE;

                // Misión de destrucción
                if (g_missions[g_game.secret_mission].jugadores[pJ_enemigo->color] == pJ_enemigo->color)
                        return TRUE;

                // Por continente
                if (g_missions[g_game.secret_mission].continentes[pP->continente] > 0)
                        return TRUE;
...
TEG_STATUS ai_puntaje_atacar( int country )
{
	...
	for(i=0;i<COUNTRIES_CANT;i++) {
		if( countries_eslimitrofe(country,i) && g_countries[i].numjug !=WHOAMI() ) {
			...
			if ( country_inmission( pP, WHOAMI() ) == TRUE){
				p_tmp += 10;
				fprintf(stderr, _("País: %s, Jugador: %i. Puntaje +10.\n"),g_countries[i].name,WHOAMI() );
			}
			else {
				fprintf(stderr, _("País: %s, Jugador: %i. Puntaje nada.\n"),g_countries[i].name,WHOAMI() );
			}
