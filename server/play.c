...
struct {
	...
	{ TOKEN_FICHAS,		token_fichas,	N_("to place the initials 8 armies") },
	{ TOKEN_FICHAS2,	token_fichas2,	N_("to place the initials 4 armies") },
	...
	{ TOKEN_EJER2,		token_ejer2,	N_("to place 3 armies in a country. You must have that card") },
...
STATIC TEG_STATUS token_fichasc( int fd, char *str )
{
	...
	/* Me fijo que no tenga todos los paÃ­ses bloqueados.
	Si tiene todo bloqueado pongo el total de fichas en 0 (no funciona aÃºn). */
	player_listar_countriesbloqueados( pJ, &countries_bloqueados );
	//fprintf(stderr,_("PaÃ­ses totales: %i vs. PaÃ­ses bloqueados: %i\n"),pJ->tot_countries,countries_bloqueados);
	if ( pJ->tot_countries == countries_bloqueados ) {
		pJ->fichasc_armies = 0;
		pJ->fichasc_conts = 0;
		total_armies = 0;
	}
...
STATIC TEG_STATUS token_attack( int fd, char *str )
{
	...
	if( conq == 1 ) {
		...
		if( player_is_lost( pJ_dst ) ) {
			...
			// Cambio el objetivo de aquellos que tuvieran que destruir a este jugador como objetivo
			int i, numjug_destruir;
			...
			while( !IsListEmpty( &g_list_player ) && (l != &g_list_player) ) {
				...
				// Busco a qué jugador tenía que destruir
				for(i=0;i<TEG_MAX_PLAYERS;i++)
				{	
					if( g_missions[pJ_tmp->mission].jugadores[i] ) {
						player_verificardestruir( i, pJ_tmp, &numjug_destruir );
					
						// No voy a cambiar el objetivo ni del que perdió ni del que conquistó.
						//fprintf(stderr,_("pJ_src: %i, pJ_dst: %i, pJ_tmp: %i, numjug_destruir: %i\n"),pJ_src->numjug,pJ_dst->numjug,pJ_tmp->numjug,numjug_destruir);
						if( pJ_src->numjug != pJ_tmp->numjug && pJ_dst->numjug != pJ_tmp->numjug && numjug_destruir == pJ_dst->numjug )
						{
								//cambio el objetivo por el objetivo común
								pJ_tmp->mission=MISSION_COMMON;
						}	
					}
				}
				l = LIST_NEXT(l);
...
STATIC TEG_STATUS token_card( int fd, char *str )
{
	...
	if( pJ->tot_exchanges > 2 && pJ->turno_conq < 2 )
		goto error;
	 ...
	 * le agrego 3 fichas automaticamente como dice el reglamento.
	 */
	if( pP->numjug == pJ->numjug && !country_esbloqueado ( pP, pJ->numjug ) ) {
		pP->ejercitos += 3;
		pJ->tot_armies += 3;
	...
	} else if( country_esbloqueado ( pP, pJ->numjug ) ) {
		con_text_out(M_INF,_("País bloqueado, (%s). No se pueden agregar 3 ejercitos.\n"), pP->name);
	}
...
/* Place 3 armies in the card's country. The player must own the country */
STATIC TEG_STATUS token_ejer2( int fd, char *str )
{
	...
	if( g_countries[ country ].numjug == j->numjug && g_countries[ country ].tarjeta.numjug == j->numjug ) {
		g_countries[ country ].ejercitos += 3;
		j->tot_armies += 3;
