...
TEG_STATUS aux_token_fichasc( PSPLAYER pJ )
{
	...
	/* Me fijo que no tenga todos los países bloqueados.
	Si tiene todo bloqueado pongo el total de fichas en 0 (no funciona aún). */
	player_listar_countriesbloqueados( pJ, &countries_bloqueados );
	fprintf(stderr,_("Países totales: %i vs. Países bloqueados: %i\n"),pJ->tot_countries,countries_bloqueados);
	if ( pJ->tot_countries == countries_bloqueados ) {
		armies = 0;
                conts = 0;
		x_canje = 0;
		//fprintf(stderr,_("Ejércitos: %i, por continente: %i, x_canje: %i\n"),pJ->fichasc_armies,pJ->fichasc_conts,x_canje);
		fichasc_next();
	}
	fprintf(stderr,_("Ejércitos: %i, por continente: %i, x_canje: %i"),pJ->fichasc_armies,pJ->fichasc_conts,x_canje);
