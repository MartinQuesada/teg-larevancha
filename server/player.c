...
/* given a color, return the player who owns it */
TEG_STATUS player_whoiscolor( int color, PSPLAYER *j)
{
	PLIST_ENTRY l = g_list_player.Flink;
	PSPLAYER i;
...
/* given a player, returns the player he must destroy */
TEG_STATUS player_verificardestruir( int color, PSPLAYER pJ, int *numjug )
{
	int f;
	PSPLAYER pJ_temp;

	f=player_whoiscolor( color, &pJ_temp );
	//fprintf(stderr,_("Cantidad de jugadores: %i, Jugador: %i, Objetivo: destruir al color %s(%i), jugador numero %i\n"),g_game.players,pJ->numjug,g_colores[color],color,f);
	// Ver que exista y que no sea destruirse a sí mismo
	if( f==pJ->numjug || player_whois( f, &pJ_temp ) == TEG_STATUS_PLAYERNOTFOUND )
	{
		// Si no existe o es él mismo lo mando al de la derecha
		f=pJ->numjug+1;
		// El jugador tiene que existir. Sino lo mando al primero de la ronda. Si el jugador es el primero de la ronda lo mando al segundo.
		if( f > g_game.players-1 ) {
			if (pJ->numjug != 0) {
				f=0;
			} else {
				f=1;
...
/* quantity of blocked countries that a player has */
TEG_STATUS player_listar_countriesbloqueados( PSPLAYER pJ, int *countries_bloqueados )
{
	PLIST_ENTRY list;
	PCOUNTRY pP;
	int i=0;
	...
	while( !IsListEmpty( &pJ->countries ) && (list != &pJ->countries ) ) {
		pP = (PCOUNTRY) list;
		if( country_esbloqueado( pP, pJ->numjug ) ) {
			i++;
		}
